...
class TestRegisterView:
    ...
    def test_returns_201_on_success(self, api_client):
        with patch("apps.accounts.tasks.send_otp_email.delay"):
            res = api_client.post(self.url, {"email": "new@example.com", "password": "securepass123", "role": "mother"})
        assert res.status_code == 201

    def test_returns_400_on_duplicate_email(self, api_client):
        MotherUserFactory(email="dup@example.com")
        with patch("apps.accounts.tasks.send_otp_email.delay"):
            res = api_client.post(self.url, {"email": "dup@example.com", "password": "securepass123", "role": "mother"})
    ...
    def test_returns_400_on_invalid_role(self, api_client):
        res = api_client.post(self.url, {"email": "x@x.com", "password": "pass123", "role": "wizard"})
        assert res.status_code == 400

    def test_does_not_require_auth(self, api_client):
        with patch("apps.accounts.tasks.send_otp_email.delay"):
            res = api_client.post(self.url, {"email": "open@example.com", "password": "securepass123", "role": "mother"})
...
class TestVerifyOTPView:
    ...
    def test_activates_user_and_returns_tokens(self, api_client):
        with patch("apps.accounts.tasks.send_otp_email.delay"):
            services.register_user(email="v@example.com", password="pass123", role="mother")
    ...
    def test_returns_401_on_wrong_otp(self, api_client):
        with patch("apps.accounts.tasks.send_otp_email.delay"):
            services.register_user(email="bad@example.com", password="pass123", role="mother")
...
class TestLoginView:
    ...
    def test_returns_tokens_on_valid_credentials(self, api_client):
        ...
        user.save()
        res = api_client.post(self.url, {"email": "login@example.com", "password": "testpass123"})
        assert res.status_code == 200
        assert "access" in res.data

    def test_returns_401_on_wrong_password(self, api_client):
        MotherUserFactory(email="wrongpw@example.com")
        res = api_client.post(self.url, {"email": "wrongpw@example.com", "password": "wrongpass"})
        assert res.status_code == 401

    def test_returns_401_for_inactive_user(self, api_client):
        InactiveUserFactory(email="inactive@example.com")
        res = api_client.post(self.url, {"email": "inactive@example.com", "password": "testpass123"})
        assert res.status_code == 401
...
class TestProfileImageView:
    ...
    def test_uploads_valid_image(self, auth_client):
        ...
        with patch("cloudinary.uploader.upload", return_value={"public_id": "x", "secure_url": "https://img.com/x.jpg"}):
